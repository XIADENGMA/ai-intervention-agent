...
| R704    | `tests/test_lottie_reduced_motion_rest_frame_r704.py`                   | 模式 C + 无障碍契约              | **减弱动态下 Lottie 静止帧保护（移动端永远降级的根因）**。锁定 `prefers-reduced-motion` 开启时仍加载 Lottie 嫩芽（`autoplay: !_prefersReducedMotion()`）并静止到「完整长成」帧 `SPROUT_REST_FRAME`（42，逐帧截图核实的 36-48 稳定段内；71 帧是循环回起点的空土丘），而非降级到简笔 SVG——iOS「减弱动态效果」用户此前永远看不到 Lottie 画面。同时锁定 MediaQueryList 监听（含旧 WebKit `addListener` 兼容分支）实时切换播放/静止，以及 `error` 处理先销毁失败实例*再*绘制降级图（残留实例会让下一次 init 提前 return；延迟 destroy 会抹掉刚绘制的降级图）。                                                                                                                                                                                                                                        |
