...
- `auto_resubmit_timeout`: 自动重调倒计时（240-290 秒）
...
- 默认值：240秒（4分钟）
- 最大值：290秒（强制限制）
- 用途：前端自动重调前的等待时间（到时会自动提交一次空反馈以保持会话）
...
属性:
    ...
    auto_resubmit_timeout (int): 自动重调倒计时（秒），默认 240，最大 290
...
- **时间复杂度（优化后）**：
  ...
  - get_task: O(1)
  - get_all_tasks: O(n)
  - remove_task: 最坏 O(n)（删除活动任务时需遍历查找下一个任务；否则为 O(1)）
...
参数:
    ...
    predefined_options (Optional[List[str]]): 预定义选项列表
        ...
        - 空列表也表示无选项
        - 示例：["同意", "拒绝", "需要更多信息"]
    auto_resubmit_timeout (int): 自动重调倒计时（秒）
        - 默认值：240秒（4分钟）
        - 最大值：290秒（自动限制）
        - 前端倒计时时间
...
说明:
    ...
    - 如果需要只读视图，可考虑对 Task 使用拷贝（例如 dataclasses.replace）
