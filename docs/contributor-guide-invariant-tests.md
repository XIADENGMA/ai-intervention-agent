...
| R704    | `tests/test_lottie_reduced_motion_rest_frame_r704.py`                   | Pattern C + a11y contract   | **Reduced-motion Lottie rest-frame guard (mobile always-fallback root cause)**. Locks `prefers-reduced-motion` to still load the Lottie sprout with `autoplay: !_prefersReducedMotion()` and park it on the fully-grown `SPROUT_REST_FRAME` (42, inside the frame-by-frame verified 36-48 stable segment; frame 71 is the bare loop-restart mound) instead of degrading to the hand-drawn fallback SVG — iOS "Reduce Motion" users previously never saw the artwork. Also locks the MediaQueryList watcher (with legacy WebKit `addListener` branch) toggling play/rest live, and the `error` handler destroying the failed instance _before_ painting the fallback (a stale instance made the next init return early; deferred destroy wiped the fresh fallback).                            |
