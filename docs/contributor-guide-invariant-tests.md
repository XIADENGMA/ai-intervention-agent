...
| R696    | `tests/test_lottie_eager_countdown_icon_r696.py`                          | Pattern C + template/CSS   | **Eager-Lottie & themed countdown-icon guard**. Locks lottie.min.js to load via first-paint `<script defer>` before app.js (empty-state animation starts as Lottie, no SVG-fallback hot-swap; fallback only for reduced-motion / load failure) and the countdown label to be an inline `currentColor` SVG clock instead of the ⏰ emoji. |
