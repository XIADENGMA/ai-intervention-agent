...
| R690    | `tests/test_vscode_countdown_controls_parity_r690.py`                   | Pattern C + webview        | **VSCode countdown-controls parity guard**. Locks the webview to render +60s/freeze countdown controls wired to the server extend/freeze endpoints, matching the web page's capability surface. |
...
| R695    | `tests/test_web_countdown_header_visibility_r695.py`                     | Pattern C + CSS/JS         | **Web countdown-controls visibility & freeze-semantics guard**. Locks `.header-info-container` to stay visible (chip / +60s / freeze anchors live inside it), keeps the freeze success path deregistering the countdown entry, and requires the explicit disabled-timeout guard before rebuilding countdowns — so freezing never auto-submits. |
