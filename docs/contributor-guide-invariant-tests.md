...
| R448    | `tests/test_feat_i18n_zh_tw_untranslated_negative_validation_r448.py`   | Pattern B + meta-invariant  | **R366 (main app zh-TW untranslated audit) 负面自验证 — i18n meta-invariant 子模式第 3 次应用 + 元方法学层第 13 应用 → 完全工业化深化期 + 2**. cycle-52 #B1. 合成 5 种 zh-TW locale drift 场景 (100%/50%/balanced/near-ceiling 7%/\_meta 元数据过滤), 反向验证 R366 的 `_flatten_strings` helper + 8% ceiling 算法在 zh-TW locale 漂移场景能正确 fire。**i18n meta-invariant 子模式从 2 应用 (R442) → 3 应用 (R448) 进入工业化阈值** (与 doc-parity 6 应用 / API contract 1 应用形成可比演化节奏)。zh-TW (繁体中文) 是台湾/香港 IDE 用户群关键 locale, R366 静默失效 = 漏译可能直接 ship 到 marketplace 伤台港用户。R448 完全复用 R442 模板 + zh-TW 适配, 证明 i18n 子模式有 _机械化复用_ 能力。                                                                                              |
...
| R673    | `tests/test_strip_images_result_copy_r673.py`                           | Pattern B + regression      | **Strip-images result copy regression guard**. Locks the result-copy path so image stripping preserves the expected text payload while avoiding stale image data propagation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| R674    | `tests/test_task_queue_watchdog_record_copy_r674.py`                    | Pattern B + concurrency     | **TaskQueue watchdog record-copy guard**. Ensures watchdog diagnostics snapshot task records instead of exposing mutable queue internals across monitoring paths.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| R675    | `tests/test_import_config_single_snapshot_r675.py`                      | Pattern C + config          | **Import-config single-snapshot guard**. Locks configuration import to one coherent snapshot so validation and application cannot diverge during concurrent changes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| R676    | `tests/test_network_security_update_copy_r676.py`                       | Pattern B + security        | **Network-security update copy guard**. Ensures network security updates copy incoming structures before mutation, preventing caller-owned state from leaking into runtime policy.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| R677    | `tests/test_config_validate_section_copy_r677.py`                       | Pattern B + config          | **Config section validation copy guard**. Keeps section validation side-effect free by checking copied data rather than mutating shared configuration dictionaries.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| R678    | `tests/test_restore_config_copy_r678.py`                                | Pattern B + config          | **Restore-config copy guard**. Ensures restored configuration state is copied at boundaries so rollback paths cannot retain mutable aliases.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| R679    | `tests/test_state_machine_transitions_copy_r679.py`                     | Pattern B + state machine   | **State-machine transition copy guard**. Locks transition metadata copying so callers cannot mutate stored transition state after registration.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| R680    | `tests/test_notification_routes_get_section_direct_r680.py`             | Pattern C + route contract  | **Notification route section-access guard**. Keeps notification routes on the direct section access path expected by the optimized configuration flow.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| R681    | `tests/test_server_print_config_web_ui_copy_r681.py`                    | Pattern B + CLI/Web UI      | **Server print-config Web UI copy guard**. Ensures printed Web UI configuration data is copied before presentation, preventing display helpers from mutating live config.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| R682    | `tests/test_notification_cleanup_tuple_snapshots_r682.py`               | Pattern B + cleanup         | **Notification cleanup tuple snapshot guard**. Locks cleanup snapshots to tuple-based immutable records so cleanup iteration is not invalidated by concurrent mutation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| R685    | `tests/test_wait_completion_survives_client_close_r685.py`              | Pattern C + regression      | **Wait-completion client-close survival guard**. Locks `wait_for_task_completion` to per-request pooled-client lookup so a config hot-reload closing the httpx client can never strand a waiting session and lose user feedback.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| R687    | `tests/test_multi_task_render_idempotent_skip_r687.py`                  | Pattern C + frontend        | **Multi-task render idempotent-skip guard**. Locks description/options renders to dataset-signature short-circuits so unchanged poll/SSE refreshes never rebuild DOM (no MathJax/Prism flicker, no lost selection).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| R688    | `tests/test_vscode_marked_renderer_merge_r688.py`                       | Pattern C + webview         | **VSCode marked renderer-merge guard**. Locks the webview marked config to `marked.use({renderer})` partial merge; `setOptions({renderer})` replaces the whole Renderer and breaks headings/lists/tables on marked v15.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| R689    | `tests/test_wait_completion_deadline_extension_r689.py`                 | Pattern C + timeout         | **Backend deadline-extension awareness guard**. Locks the backend wait loop to probe `remaining_time` before timing out so user extends / typing auto-extends never trigger a premature ghost-close.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| R689    | `tests/test_typing_hold_and_autosubmit_content_r689.py`                 | Pattern C + frontend        | **Typing-hold & auto-submit-content guard**. Locks web + webview countdowns to auto-extend while the user is typing and to submit typed text/options (not the resubmit prompt) when the countdown reaches zero.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| R690    | `tests/test_vscode_countdown_controls_parity_r690.py`                   | Pattern C + webview         | **VSCode countdown-controls parity guard**. Locks the webview's +60s/freeze DOM + extend/freeze endpoint wiring to stay present, while (since R700) the controls row is intentionally `display: none` — typing-hold auto-extension replaces the manual buttons; also locks the zero-tick typing guard and the header_label-first tab label.                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| R691    | `tests/test_webview_task_fields_parity_r691.py`                         | Pattern C + API contract    | **Task-level field parity guard**. Locks `/api/config` to return `feedback_placeholder` / `question_type` / `header_label` and the webview to consume them (chip, placeholder override, yes/no group), matching the web page.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| R692    | `tests/test_submit_focus_and_notify_deeplink_r692.py`                   | Pattern C + UX flow         | **Submit-focus & notify-deep-link guard**. Locks the post-submit auto-focus request/consume flow (web + webview, freshness-windowed, yes/no-aware) and the hidden-webview notification deep-link that switches to the notified task on reveal.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| R695    | `tests/test_web_countdown_header_visibility_r695.py`                    | Pattern C + CSS/JS          | **Web countdown freeze-semantics guard**. Keeps the freeze success path deregistering the countdown entry and requires the explicit disabled-timeout guard before rebuilding countdowns — so freezing never auto-submits. Since R700 the `.header-info-container` row is intentionally hidden (the tab ring is the countdown's only visual home) and the test locks that retirement.                                                                                                                                                                                                                                                                                                                                                                                                          |
| R696    | `tests/test_lottie_eager_countdown_icon_r696.py`                        | Pattern C + template/CSS    | **Eager-Lottie & themed countdown-icon guard**. Locks lottie.min.js to load via first-paint `<script defer>` before app.js (empty-state animation starts as Lottie, no SVG-fallback hot-swap; since R704 the fallback is reserved for runtime load failure only) and the countdown label to be an inline `currentColor` SVG clock instead of the ⏰ emoji.                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| R702    | `tests/test_task_timeout_explicit_guard_r702.py`                        | Pattern A + source contract | **Explicit per-task timeout hot-reload guard (ghost-submit root cause)**. Locks `Task.auto_resubmit_timeout_explicit`: tasks whose timeout was explicitly provided by the API caller are never overwritten by the `frontend_countdown` config hot-reload sync, and callback registration only records the baseline instead of running a sync (registration is not a config change).                                                                                                                                                                                                                                                                                                                                                                                                           |
| R703    | `tests/test_macos_xdg_hijack_guard_r703.py`                             | Pattern A + config paths    | **macOS XDG-hijack & same-file migration guard (config-wipe root cause)**. Locks the macOS standard config dir to the Apple-convention path even when `XDG_CONFIG_HOME` is exported (platformdirs >= 4.5.0 would otherwise collapse it into the legacy `~/.config/...` path), and locks the R686 retire/migrate branches to skip when standard + legacy resolve to the same physical file (symlinks) — so the only real config is never renamed away and user settings survive restarts.                                                                                                                                                                                                                                                                                                      |
| R712    | `tests/test_lottie_plays_despite_reduced_motion_r712.py`                 | Pattern C + product contract | **Empty-state animations exempt from Reduce Motion (supersedes the R704 rest-frame contract)**. R704 parked the sprout on a rest frame for `prefers-reduced-motion` users; on real devices that read as "the animation is broken" (the loading bar was simultaneously flattened to a one-shot 0.01ms run by the global reduce rule). R712 locks the maintainer's on-device decision: the empty-state Lottie sprout runs `autoplay: true` unconditionally, the R704 preference machinery (watcher / rest-frame helper) is fully removed, and main.css keeps the `.no-content-progress-bar` exemption (`animation-iteration-count: infinite !important`) inside the global reduce block; the `error`-handler destroy-before-fallback ordering contract carries over from R704 unchanged. Every other animation keeps honoring the system preference.                            |
| R713    | `tests/test_close_terminal_state_r713.py`                                | Pattern C + i18n keys       | **"Close Web UI" terminal-card guard**. The old flow force-reloaded 2s after `/api/close`, but the server shuts down 0.5s in — the reload could only land on the SW's offline.html ("cannot connect" + a forever-useless Retry button) or the browser's native error page, even though the user closed the UI on purpose. Locks: the success branch calls `renderClosedTerminalState()` to render an in-place success-semantics card and `return`s (never reloads) while the failure branch keeps the refresh fallback; the renderer must stop SSE (full `_disconnectSSE` cleanup), dispose the Lottie lifecycle, swap in `status.closedTitle/closedHint` (stripping `data-i18n` so a late translateDOM can't overwrite — the R709 lesson), and hide the progress bar / close button / SSE badge; all three locales must ship the terminal keys with `closedRefreshing` removed alongside the behavior.                            |
| R714    | `tests/test_notification_electron_webview_fallback_r714.py`              | Pattern C               | **Electron-webview notification fallback guard** (Cursor / VS Code built-in browser). In Electron webviews, service-worker `showNotification()` commonly *silently fails* — the promise resolves but no notification is shown (electron#13041 / #10146); the old code treated the resolve as success and never tried the page-level `new Notification()`, so notifications were lost. Locks the two defense layers in `showSystemNotification`: (1) `isElectronHost()` UA probe (`Electron/` token) skips the SW path entirely and goes straight to the page-level constructor (which Electron maps to native notifications); (2) after a SW `showNotification` resolves, `getNotifications({tag})` verifies the notification actually registered — an empty result falls back to the page-level path, while a query *exception* is conservatively treated as success so SW-only platforms (Android Chrome, where the page constructor throws) keep their behavior.                            |
| R705    | `tests/test_options_render_single_source_r705.py`                       | Pattern C + JS contract     | **Predefined-options single render source guard (body-only page root cause)**. Locks `loadConfig` delegating options rendering to `updateOptionsDisplay` (multi_task.js loads before app.js — template order also locked); the guarded local fallback must clear the container and toggle `hidden`/`visible` classes, never inline `style.display` (the container's initial `.hidden` is `display:none !important`, so inline block left options in the DOM but invisible). Also locks the `lastLoadedDetailsTaskId` watermark: polling retries `loadTaskDetails` every cycle until the active task's details have rendered successfully once — the old condition never retried in the pending-task scenario, leaving the page body-only forever after a single failed fetch.                |
| R706    | `tests/test_bark_custom_scheme_url_r706.py`                             | Pattern A + provider contract | **Bark custom-scheme tap-through guard (shortcuts:// deep links)**. Locks `_is_acceptable_bark_click_url` accepting any RFC 3986 `scheme://` URL (so `bark_url_template = "shortcuts://run-shortcut?name=..."` reaches the phone) while `javascript:` / `data:` (no authority) stay rejected; loopback suppression only applies to http(s) (`shortcuts://localhost` must not be misclassified, `http://localhost:8080` keeps being suppressed); metadata URL candidates are validated the same way with garbage values falling back to the template.                                                                                                                                                                                                                                          |
| R707    | `tests/test_ios_a2hs_server_dismiss_r707.py`                            | Pattern A + config/endpoint | **iOS A2HS banner server-side dismiss guard (Shortcuts WebView localStorage loss)**. Locks the `web_ui.ios_a2hs_hint_dismissed` config field (default false), the idempotent any-origin `POST /api/system/ios-a2hs-dismiss` endpoint (banner only shows on remote iOS devices; already-true skips the config write), the `window.AIIA_IOS_A2HS_DISMISSED` template injection, and the frontend contract that the injected flag is checked before localStorage and the dismiss handler fire-and-forget POSTs back — SFSafariViewController neither shares nor persists localStorage, so a client-only dismiss reappeared on every Shortcuts visit.                                                                                                                                             |
| R709    | `tests/test_i18n_dynamic_content_guard_r709.py`                         | Pattern C + i18n contract   | **translateDOM dynamic-content overwrite guard (prompt-wiped-to-"Loading…" root cause)**. `translateDOM()` rewrites `textContent`/placeholder of every `[data-i18n*]` element on language switch and on late `ensureDefaultLocale()` completion — wiping the rendered task prompt (`#description` ships `data-i18n="page.loading"`) permanently (the R687 render signature still matched, so polling short-circuited). Locks: both prompt render entry points strip `data-i18n` before writing real content; per-task feedback placeholder removes/restores `data-i18n-placeholder` two-way; custom-sound status and clipboard hint sync the attribute to the concrete i18n key (helper returns `{key, text}` with literal `t("…")` calls for the orphan-key scanner); `#countdown-text` carries no `data-i18n` (per-second interpolated rewrite). |
| R710    | `tests/test_stale_precompressed_fallback_r710.py`                       | Pattern A + HTTP contract   | **Stale precompressed-asset fallback guard (immutable cache poisoning root cause)**. `_send_with_optional_gzip` picked `.br`/`.gz` siblings by existence only — in the minify-without-precompress window a Brotli-capable browser received old bytes under a fresh `?v=<mtime>` URL with `Cache-Control: immutable`, pinning stale code in the cache for a year with no self-healing (bit during R709 verification). Locks the freshness rule: a compressed sibling is used only when its mtime is >= the source's; stale `.br` falls through to fresh `.gz`, and stale-everything falls back to the identity source file.                                                                                                                                                                     |
