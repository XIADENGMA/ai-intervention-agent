...
- [x] 参考<https://github.com/Pursue-LLL/interactive-feedback-mcp>添加功能
  ...
  1. ✅ **task_id 自动生成** - 系统自动生成唯一 task_id（使用时间戳+随机数），不再支持手动指定，彻底避免冲突问题
  2. ✅ **自动重调机制** - 自动在规定时间内重新调用工具以延续对话 - Cursor 等 IDE 有会话超时时间限制，当超时后在该工具中输入提交后会直接报错，在超时时间内进行自动重新调用可以使会话始终保持活跃，持续等待用户输入新的下一步指令，从而达到一次请求在较长一段时间内仍能完成多次任务的需求
  ...
  3. ✅ **项目标识显示** - 显示当前项目以用于多窗口时的区分 - 便于在多项目同时开发时快速识别当前操作的项目
...
- 修复问题：如果我现在依次在两个对话里面调用 interactive_feedback，它现在在 8081 页面只显示了一个标签，而不是显示第一个标签后，当第二个对话开始调用时，显示第二个标签。
