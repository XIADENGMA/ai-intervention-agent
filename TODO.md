...
- [x] 调用时出现`Error handling CallToolRequest: McpError: MCP error -32001: Request timed out`，导致调用者的调用整个失败，但是服务还是在正常运行【已修复】0. 当前是在 mcphub 内管理多个 MCP，其中包括 ai-intervention-agent，以及其他一些 mcp，ai-intervention-agent 的 mcp 地址是`http://0.0.0.0:20004`，通过 streamable HTTP 方式进行调用
...
- [x] 请使用 uv、ruff、ty 和 pyproject.toml 更新我的项目；以及 test.py 设置--port 8080，那么里面全部都使用 8080 端口。
- [x] 仔细分析后修复问题：使用 uv run python test.py --port 8080 --verbose --thread-timeout 0，第一次反馈虽然是在 8080 端口，但是`🔄 更新页面内容...`后的下面反馈现在是根据 config.jsonc 来的
- [x] 界面截图：包括桌面端和移动端，包括深色模式和浅色模式，包括无有效内容页面、有内容页面（uv run python test.py --port 8080 --verbose --thread-timeout 0 的第一个页面）
- [x] 请积极调用交互 mcp；README 优化：badge（添加上插件以及 deepwiki）、图片（优化显示方式）、文字信息（默认是英文。在介绍功能时不需要要技术细节，在细节时才有技术细节。不需要核心功能详解。）、架构图（根据当前实现更新）
      参考：https://raw.githubusercontent.com/siddharthvaddem/openscreen/refs/heads/main/README.md
- [x] 请同步更新 README.zh-CN.md 以及去除之前的。请搜索著名开源项目的 readme 作为参考（比如：https://github.com/xmake-io/xmake、siddharthvaddem/openscreen、），优化文字描述；以及配置文件说明请单独有个说明文档（包括中英文版本）；README 内图片至少包括深色模式、浅色模式、桌面端、移动端（如何显示深色模式、浅色模式的图片根据系统主题自动切换就最好了，实现不了就不需要这个自动切换功能）
- [x] 在程序运行中，修改/home/xiadengma/.config/ai-intervention-agent/config.jsonc 的 feedback 部分，我实际测试后发现，resubmit_prompt 和 auto_resubmit_timeout 没有更新，导致我超时后/点击提交后，没有正确的提示。
- [x] mDNS 支持：使用默认是 ai.local，这个加到配置文件内，然后使用 mDNS 服务发现，然后使用 ai.local 访问。
  - 探测逻辑：会优先发布 物理网卡的 LAN IPv4，并避开 `tun` / `tailscale*` / `docker0` / `br-*` 等接口。
- [x] 现在 packages/vscode 对应打包后的插件一直在`无有效内容 连接中...`（根据我观察，插件是可以连接到对应地址的，但是插件界面没有正确显示，还是无有效内容，但是底栏的信息显示有任务），而且那个嫩芽生长动画是 emoji 而不是对应的动画（好像是回退情况），请仔细分析和深度思考以及网络搜索，并且考虑边界情况后修复
- [x] 仔细分析和深度思考以及网络搜索，并且考虑边界情况，以最佳实践方案优化`packages/vscode`文件夹下的插件日志（精简、必要、高效、清晰）
- [x] 当前 vscode 插件没有一个正确、完整、有效的自动测试流程，请仔细分析和深度思考以及网络搜索，用最佳实践方案给我实现
- [x] 复制、粘贴优化，特别是在 ios 平台上和插件上
- [ ] 优化 prompt
- [x] 优化代码 /code-simplifier
- [x] 插件添加支持：使用 Node.js 的 child_process 执行 AppleScript，同时在可能需要的地方添加对应的设置/说明。
- [x] 现在整个通知模块搞得很混乱，有什么通知、Web通知、Bark 通知、MacOS 原生通知这些。首先你要知道我现在一个核心（核心程序）+2 种显示方式（网页和插件），然后呢，网页可能会在移动端、桌面端查看，而插件我只安装在 vscode、cursor 这些桌面 IDE 上面（但是可能是 windows、linux、macos 这些系统）。请仔细分析和深度思考以及网络搜索，并且考虑边界情况，以最佳实践方案优化通知模块。
  - 通知情况树：
    1. 网页通知：
       1. 移动端：包括 android 和 ios
       2. 桌面端：包括 windows、linux、macos
    2. 插件通知：
       1. 桌面端：包括 vscode、cursor 等
    3. Bark 通知：只传到指定 ios 设备上
  - 阶段 A（Python 核心，先稳住架构）：在 notification_manager.py/notification_providers.py 建立 BaseProvider 抽象、统一 NotificationEvent（补 priority 等）、完善智能路由/降级/统计（delivery_success_rate 等）与 EnhancedLogger 可观测性。
  - 阶段 B（Web 侧收敛）：清理 static/js/multi_task.js 过时通知逻辑，统一改为通过一个前端“通知中心”入口派发事件；桌面端走 Visual Hint，移动端按配置优先 Bark（必要时补一个后端事件触发端点）。
  - 阶段 C（VSCode 插件侧收敛）：把目前 Webview→Extension 的通知链路重构为统一的 NotificationEvent 分发（VSCode API / AppleScript provider），并确保 enableAppleScript 与 onDidChangeConfiguration 热更新一致。
- [x] 网页通知问题检查与修复：之前在 macos 上面无法正常的通知，从 ai.local:port或者 127.0.0.1:port 或者 localhost:port或者 0.0.0.0:port 访问页面，到获取权限到触发通知，最后只有页面内部通知，而没有其他通知（除了 Bark 通知）。现在请检查问题是否还存在，存在的话，进行对应的修复。
- [x] 网页深色模式/浅色模式问题修复：测试发现，macos 系统上 ai.local:port 访问页面，默认浅色问题没问题，但是点击切换按钮，只有卡片的部分变成了深色模式，而除了卡片以外的地方，没有变成深色模式。
- [x] 对整个项目进行全面的注释优化，非必要不保留，并且全部使用中文注释。
- [x] 对整个项目进行全面的命令/文本优化，从专业性、准确性、可阅读性、一致性等方面入手。
- [x] 请完整的根据当前代码实现更新 readme，特别是里面的 mermaid 图表，要正确、完整、有效。
- [x] 插件还能不能优化，包括插件的 UI 和插件的交互。
- 先根据TODO.md的# WorkFlow的Web UI/交互真机验证和提交前（本地 CI Gate）进行本地验证，确认正确后，先根据我之前的 commit 风格进行 commit 一次（是不是要先 git pull一次），但是暂时先不提交。

# CHECK

**仔细分析和深度思考以及网络搜索，并且考虑边界情况，依次检查当前全部代码和文档有没有逻辑上的错误。**

**仔细分析和深度思考以及网络搜索，并且考虑边界情况，依次检查当前全部代码和文档有没有低级错误。**

**仔细分析和深度思考以及网络搜索，并且考虑边界情况，依次检查当前全部代码和文档有没有一致性错误。**
...
- **提交前（本地 CI Gate）**
  ...
  - VSCode 插件：`npm run vscode:check`（Linux/headless 可用：`xvfb-run -a npm run vscode:check`）
...
- **发布（tag 触发 GitHub Actions Release）**
  ...
  - 若发布流水线失败：**修复后 bump 补丁版本再重新打 tag（例如从 `v1.4.17` → `v1.4.18`）**，不要移动/重打已发布的 tag
