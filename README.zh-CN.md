...
- 🔁 **自动重调** —— 保持长会话不被客户端硬超时切断；归零时优先提交已输入的文本与勾选项，绝不发空提示
- ⏱️ **输入即延长** —— 正在输入时倒计时自动延长、归零也绝不打断（Web 页面与 VS Code 插件语义一致）
- 🔔 **通知** —— Web UI / 声音 / 系统通知 / Bark，支持上传自定义通知音效
- 🌐 **SSH / 局域网友好** —— 适配 SSH 端口转发；本地网络支持时通过 mDNS 发布 `<host>.local` 入口
- 🏷️ **Header 标签 & Yes/No 按钮** —— 每任务 `header_label` 上下文短标签；`question_type='yesno'` 渲染一键二元决策
- 🎨 **自定义占位提示** —— 每任务 `feedback_placeholder` 覆盖默认输入框占位文本
- 🌏 **i18n** —— Web UI + VS Code 插件原生支持 `en` / `zh-CN` / `zh-TW` 三语
- ⚡ **生产力快捷键** —— 按 `?` 弹出快捷键速查表；per-task 草稿自动保存；可配置提交模式；实时字符计数器
- 💬 **常用回复 / 快捷短语** —— 常用反馈一键插入，支持 JSON 导出 + 导入
- 🟢 **SSE 实时状态指示** —— 角落 3 态徽章（绿/橙/红）提示与后端的同步状况
- 📱 **PWA + 离线可用** —— 可从浏览器原生入口安装；品牌化离线页 + 服务恢复自动 reload，替代浏览器默认错误页
- ♿ **WCAG 2.1 AA 无障碍** —— 对比度、Name/Role/Value、焦点管理、`prefers-reduced-motion` / `prefers-contrast: more` 均经审计并由不变量测试锁定
- 🛡️ **稳定安装** —— 基于 Flask 3.x + 保守的依赖锁版；免疫 2026 年初 [Starlette 1.0 breaking change](https://github.com/Minidoracat/mcp-feedback-enhanced/issues/213)（该 bug 让若干同类项目默认安装即报错）
...
- 另有单一 MCP 工具接口、AST-based 锁获取顺序契约（静态证明无死锁环）、
  lazy-init 审计等——完整目录见
  [`docs/contributor-guide-invariant-tests.zh-CN.md`](docs/contributor-guide-invariant-tests.zh-CN.md)。
...
- **输入即延长（typing-hold）** — 正在输入时倒计时自动延长、归零也
  绝不打断
- **常用回复短语** — 常用回复保存到 `localStorage`，一键插入
...
包含当前加载的配置路径、各非敏感 section（secret 类字段自动 redact）
与生效的 env 覆盖。`network_security` 被 `ConfigManager.get_all()`
边界过滤（与 [`/api/system/health`](docs/configuration.zh-CN.md#环境变量覆盖)
同信任级）。
