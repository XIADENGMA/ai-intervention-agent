function __domSecT(key,params){try{if(typeof window!=='undefined'&&window.AIIA_I18N&&typeof window.AIIA_I18N.t==='function'){return window.AIIA_I18N.t(key,params)}}catch(_e){}
return key}
function __domSecFormatNumber(value,options){try{if(typeof window!=='undefined'&&window.AIIA_I18N&&typeof window.AIIA_I18N.formatNumber==='function'){return window.AIIA_I18N.formatNumber(value,options)}}catch(_e){}
var digits=options&&typeof options.maximumFractionDigits==='number'?options.maximumFractionDigits:2
...
for(let index=0;index<attributeEntryCount;index+=1){if(!(index in attributeEntries))continue
...
openImageModal(src,imageItem.name||'',imageItem.size||0)}}catch(e){console.warn('Open image preview failed:',e)}})
...
info.textContent=__domSecT('status.sizeLabelKB',{name:imageItem.name,size:__domSecFormatNumber(imageItem.size/1024,{maximumFractionDigits:1})})
...
button.classList.remove('copied')},2000)}catch(err){button.innerHTML=`${errorIconSvg}${copyFailedLabel}`
