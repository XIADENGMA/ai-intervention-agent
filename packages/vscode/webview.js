...
class WebviewProvider {
  ...
  _getHtmlContent(webview) {
    ...
    const markedJs = this._getMarkedJs()
    const prismJs = this._getPrismJs()
    const prismCss = this._getPrismCss()
...
<head>
    ...
    <meta http-equiv="Content-Security-Policy" content="default-src 'none'; connect-src ${serverUrl} ${cspSource}; style-src 'unsafe-inline'; script-src 'nonce-${nonce}'; img-src data: ${serverUrl} https: ${cspSource}; font-src ${serverUrl} data: ${cspSource};">
...
<body>
    ...
    <script nonce="${nonce}">${prismJs}</script>
    ...
    <script nonce="${nonce}">
        (function() {
            ...
            const LOTTIE_LIB_URL = '${lottieJsUri}';
            const NO_CONTENT_LOTTIE_JSON_URL = '${noContentLottieJsonUri}';
            ...
            async function submitWithData(text, options, taskIdOverride) {
                ...
                uploadedImages.forEach((imageData, index) => {
                    ...
                    if (response.ok) {
                        ...
                        logError('提交失败: HTTP ' + response.status);
                        vscode.postMessage({
                            type: 'showInfo',
