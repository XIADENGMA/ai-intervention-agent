...
- **⏰ 自动重调**：可配置倒计时（默认 290 秒），自动提交保持会话活跃
...
{
  "web_ui": {
    "host": "0.0.0.0", // Web UI 监听地址
    "port": 8080, // Web UI 端口
    "auto_resubmit_timeout": 290 // 自动重调超时时间（秒）
  }
}
```
...
- `host`：Web UI 监听地址，`0.0.0.0` 允许远程访问
- `port`：Web UI 端口号，默认 8080
- `auto_resubmit_timeout`：自动重调超时时间（秒），默认 290 秒
  - 倒计时结束后自动提交默认反馈，保持 AI 会话活跃
  - 在 Web UI 中显示为圆形进度条
  - 设置为 0 可禁用自动重调功能
...
每个任务会自动生成唯一的 task_id，格式为：`<项目名>-<随机数>`

**示例**：

- `ai-intervention-agent-3609`
- `my-project-7821`

**显示效果**：

- 标签页显示为：`ai-interven... 3609`（前缀截断 + 完整数字）
- 鼠标悬停显示完整 task_id
