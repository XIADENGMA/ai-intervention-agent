...
- Repository-wide comment cleanup (maintainer decision: comments are
  ...
  flasgger route docstrings (the YAML is the OpenAPI spec), the MCP
  tool docstring, RLock/ReadWriteLock concurrency contracts
  (R330/R331/R336, R22.2), decision rationales (R372 atexit grace,
  R323 reset semantics), `NOTE(feat-remove-*)` route-retention
  markers, `AIIA-XSS-SAFE` / `aiia:i18n-allow-cjk` / lint pragmas.
  ~60 invariant tests that asserted comment text now lock the
...
> ~195 R-commits, +1467 net tests + ~836 net subtests). The defining
> theme is **"industrialize the invariant-driven methodology, then turn
> it on itself"**: methodology evolves through 7 generations (v3.5 →
> v3.11) and 13 dimensions, culminating in a formal **meta-invariant
> layer (v3.11)** where invariants validate other invariants (13
> applications, 5 sub-patterns). Along the way, 8+ real production
> bugs were caught and fixed, the API surface was hardened with
> 5-dimension OpenAPI contract coverage, CI was sped up 3.76x via
> `pytest-xdist`, and several user-facing features landed (zh-TW
> locale, countdown extend/freeze, security headers strict mode).
> See [`docs/methodology-evolution.md`](docs/methodology-evolution.md)
> for the full methodology evolution SSoT, and `docs/code-reviews/cr30.md`
> … `cr82.md` for the 53 per-cycle code reviews.

### Added (user-facing features)

- **Countdown +60s extend (UI + API + race-safe core)** — UI button +
  backend `POST /api/tasks/<id>/extend` with `max_extends` /
  `min_seconds` / `max_seconds` server-config caps and full race-safe
  `TaskQueue.extend_task_deadline` facade. Idempotent under retries.
- **Freeze / unfreeze task lifecycle ops** — UI + backend `POST
  /api/tasks/<id>/freeze` with explicit idempotency declaration in
  docstring (R313, R372 decision-three-layer locked) and full
  conflict/race semantics.
- **Multi-task UI** — concurrent task viewer (`multi_task.js`) with
  SSE-backed live updates, freeze/extend per-row controls, and
  per-task progress.
- **SSE connection-status indicator** — 3-state visual (good /
  degraded / offline) corner badge. Non-intrusive, honors
  `prefers-reduced-motion`.
- **Settings panel with backdrop-close** — full settings UI
  (`settings-manager.js`) with backdrop tap-to-close, focus capture,
  and accessibility hooks.
- **Image upload UI** — paste / drag / file-picker upload of inline
  images with per-config size limits, validated against backend
  consumption (R276 cross-source consistency invariant).
- **Theme cross-tab sync** — dark/light/system theme toggle that
  syncs across tabs via `BroadcastChannel`, honors
  `prefers-color-scheme`.
- **Export tasks UI** — export task transcripts to JSON / Markdown
  (later removed in cycle-23 R274 after UX feedback).
- **iOS A2HS hint banner** — Add-to-Home-Screen prompt for iOS Safari
  users, dismissible, locale-aware.
- **Footer plugin links** — extensible footer with project links,
  attribution, and version display.
- **Bug-report issue template (`.github/ISSUE_TEMPLATE/bug_report.yml`)**
  — structured GitHub issue template for bug reports.

### Added (internationalization)

- **Traditional Chinese (zh-TW) locale** — Web UI + VSCode extension.
  Generated by `scripts/gen_zhtw_from_zhcn.py --all` from Simplified
  Chinese source with project-specific `PHRASE_MAP` for Taiwan UI
  conventions ("反饋" → "回饋", "加載" → "載入", etc.). 445-entry
  `CHAR_MAP_v2`.
- **i18n untranslated keys audit** — 4 locales locked (R350 main
  zh-CN, R353 VSCode zh-CN, R366 main zh-TW, R374 VSCode zh-TW),
  catches keys with identical EN ↔ translation values.
- **i18n orphan-key cross-locale validator** (R360 JS classifier
  forward resolution) — ensures every i18n key referenced in JS
  exists in all locale JSONs.
- **`_pseudo` locale** — `[!!]` / `[??]` pseudo-translation for layout
  drift / truncation visual testing.
- **Dynamic i18n reserved keys** (R291 / R295) — runtime-generated
  i18n keys (e.g. `task.{status}.label`) documented + cross-checked.

### Added (security)

- **Security headers strict mode** (R376) — locks 7 production
  ...
  against silent deletion or weakening.
- **CSP nonce 3-layer consistency** (R306) — server-emitted nonce
  must match HTML `<script nonce>` and JS event-handler attribute
  resolution. Caught real P0 CSP bug (cr61).
- **CSP directive completeness** (R386) — every `script-src` /
  `style-src` / `connect-src` directive enumerated, no implicit
  fallbacks.
- **CORS strict mode** (R390) — closed-list origin validation, no
  reflective `Access-Control-Allow-Origin: *`.

### Added (OpenAPI / API contract)

- **Endpoint OpenAPI docstring coverage** (R355) — every Flask route
  must have OpenAPI docstring with `tags` + `responses`.
- **POST endpoint error response documentation** (R358) — every POST
  must document its 4xx / 5xx error paths.
- **API tag taxonomy closed set** (R364) — tags restricted to
  Feedback / Notification / System / Tasks; no taxonomy drift.
- **Task ↔ OpenAPI field exposure parity** (R368) — every public
  Task model field must appear in `/api/tasks` GET response schema.
- **POST request body field consumption** (R378) — every documented
  request parameter must be consumed by the handler.
- **Endpoint summary quality** (R404 / R430) — every OpenAPI endpoint
  summary must be ≥10 chars and non-templated.
- **Route Mixin registration matrix** (R406 / R414) — every Mixin
  must register its routes; matrix invariant prevents silent drop.
- **OpenAPI property type completeness** (R398) — every property
  schema must declare its `type`.
- **OpenAPI required fields in properties** (R392) — every required
  field must appear in the properties block.
- **OpenAPI property description ratchet** (R412 / R418 / R426) —
  ratchet from 45% → 70% → 85% description coverage across 3
  iterations.
- **OpenAPI error response schema ratchet** (R422 / R428 / R432 /
  R436 / R440 / R446) — 6-iteration ratchet from 5.88% → 82.35%
  coverage (production-quality threshold). Each iteration adds 1-9
  new error schemas across `notification.py` / `system.py` /
  `task.py` / `feedback.py` / `static.py`.

### Added (test infrastructure / methodology — v3.5 → v3.11)

- **v3.6 — 4 patterns fully industrialized**:
  - perf-baseline (10 apps: cold-start, SSE bus, task-queue,
    notification cache, /api/submit POST, /api/tasks GET, …)
  - cross-language schema (REST / SSE / submit-feedback / 3-string
    const / 3-numeric const / 4-const, 7+ apps)
  - lifecycle-cleanup (AbortController, async task lifecycle, VSCode
    extension resource lifecycle, 4+ apps)
  - visual-architecture (3 README mermaid sequence diagrams locked)
- **v3.7 — 2 patterns fully industrialized**:
  - three-layer consistency (CSP nonce / PWA SW cache / Notification
    permission, 3+ apps)
  - decision-three-layer (CI xdist / fetch retry backoff / atexit
    grace period, 3+ apps with literal + ≥100-char inline rationale
    + single consumer)
- **v3.8 — 2 patterns**:
  - idempotent endpoint contract (freeze / activate / system
    endpoints, R313 / R318)
  - test-isolation (lazy-load mock safety, module-level cache reset
    audit, future-guard, 6+ apps: R316 / R319 / R324 / R325 / R327 /
    R352)
- **v3.9 — async race contract** (6 apps): task_queue write lock
  (R326) / notification_manager AST lock order (R328) /
  service_manager module-level lock order (R329) / RLock whitelist +
  rationale (R330) / web_ui RLock reentry chain (R331) /
  config_manager RLock reentry chain (R336).
- **v3.10 — doc-parity sub-pattern** (7 apps): troubleshooting
  (R303 / R335) / main README (R340) / VSCode README (R346) /
  contributor guide (R400) / configuration docs (R394) /
  methodology evolution (R444). Locks bilingual structure + section
  ordering + image / anchor integrity.
- **v3.10.1 — OpenAPI endpoint summary quality** (R404).
- **v3.10.2 — OpenAPI property description completeness** (R412)
  + ratchet 45% → 70% → 85% (R418 / R426).
- **v3.10.3 — OpenAPI error response schema parity** (R422)
  + ratchet 5.88% → 82.35% production-quality threshold (R428 /
  R432 / R436 / R440 / R446).
- **v3.11 — meta-invariant layer** (13 apps, 5 sub-patterns,
  formally named via R444 + `docs/methodology-evolution.md`):
  - Ratchet validation: 9 apps (R412 / R418 / R422 / R426 / R428 /
    R432 / R436 / R440 / R446)
  - Doc-parity negative validation: 1 app (R424)
  - API contract negative validation: 1 app (R430)
  - i18n meta-invariant: 3 apps (R438 main zh-CN / R442 VSCode
    zh-CN / R448 main zh-TW) — industrialization threshold
  - Mixin matrix negative validation: 1 app (R414)
- **API contract series** — 5 dimensions full industrialization
  (R355 coverage / R358 error / R364 taxonomy / R368 field / R378
  request schema).
- **Pydantic field validator coverage** (R380 NotificationConfig /
  R384 Task / R388 webui-feedback-config / R402 TOML section) —
  every `*_timeout` / `*_count` / `*_delay` / `*_volume` /
  `*_interval` pattern field must have `@field_validator` or
  whitelist exception.
- **Config default drift protection** (R370) — NotificationConfig
  (runtime) ↔ NotificationSectionConfig (TOML) default-value parity
  for 18 shared fields, with normalization (sound_volume int 0-100
  ↔ float 0-1).
- **JS event listener pairing audit** (R338) — modal/dialog keydown
  handler `addEventListener` + `removeEventListener` must pair.
- **Version quintet sync invariant** (R416) — pyproject.toml /
  CITATION.cff / package.json / packages/vscode/package.json /
  package-lock.json all locked to single version source.
- **Invariant test guide catalogue** (R227 / R231 — refreshed
  regex / hygiene; R416 catalogue entry).
- **README factual claims invariant** (R233 — refreshed regex for
  `N,NNN+` test count format).
- **Static compression audit** (R20.14d — extended for new asset
  variants).
- **VSCode extension resource lifecycle** (R342) — AbortController /
  EventEmitter / timer registration must have corresponding dispose.

### Added (performance)

- **CI 3.76x speedup** via `pytest-xdist -n 4 --dist=loadfile`
  (R305) + xdist worker decision invariant (R308).
- **9 perf-baseline locks** across HTTP + module-level critical
  paths: cold-start (R344), SSE bus (R304), task-queue (R315),
  notification cache (R307), `/api/submit` POST (R348),
  `/api/tasks` GET (R362), perf-baseline-const (R296), countdown
  DOM cache (R266 cycle-22), anti-FOUC (perf-audit cycle-3).

### Added (accessibility)

- **A11y cycle 1-8 + cycle 22-23** — focus management, WCAG
  contrast, focus ring, escape-handler delegation, image modal
  dialog/focus, status-hex hardcode audit, button primary/secondary
  WCAG, icon-button ARIA label, focus capture upgrade, custom-sound
  leak prevention, kshelp focus, prefers-contrast support, image
  modal focus, status hex hardcode.
- **Keyboard shortcuts help dialog** (`keyboard_shortcut_help.js`)
  with focus capture.
- **iOS Add-to-Home-Screen hint** (`ios_a2hs_hint.js`) — accessible
  banner, dismissible.

### Fixed (production bugs)

- **CSP nonce P0 bug** (cr61 / R306) — CSP nonce mismatch between
  server emission and JS event-handler attribute resolution caused
  silent script blocking. Three-layer consistency invariant prevents
  recurrence.
- **i18n scanner race condition** (cycle-49 R432 spillover) —
  `check_i18n_{js,ts}_no_cjk.py` `scan_file` was not race-safe under
  `xdist -n 4` parallel execution; produced intermittent CI flakes.
  Fix: race-safe `scan_file` + `R412 envelope tolerance` adjusted
  0.30 → 0.60.
- **R72A test isolation** (cycle-52 spillover) — `tests/
  test_root_logger_intercept_r72a.py` was unstable under `xdist`
  due to `importlib.reload` creating new class identities;
  `isinstance` check used stale class reference. Fix: dynamically
  reference `enhanced_logging.InterceptHandler` after reload.
- **task.py YAML quote ambiguity** (cycle-50 spillover) — OpenAPI
  YAML description string `"Task not found" 类的...` failed YAML
  parsing; fixed to use backticks for inner quotes.
- **NotificationHealthStreak flake** (cr63 / R316) — permanent fix
  via `_create_test_instance()` classmethod that builds a clean
  NotificationManager without reading config / starting workers /
  loading in-flight state. R319 extends this to formal helper API.
- **Lazy-load mock safety** (R324 / R325) — `web_ui_config_sync.py`
  + `service_manager.py` had lazy-init paths that leaked real
  instances during mocked test ordering; source-layer reset +
  conftest auto-reset fixed it.
- **`/api/system/open-config-file/info`** (R355) — missing OpenAPI
  `tags` and `responses` fields; invariant caught + fixed.
- **`/api/submit` 4xx error response documentation** (R358) —
  missing 4xx error response docs; invariant caught + fixed.
- **`/api/tasks` GET** (R368) — missing 5 user-visible fields
  (`completed_at`, `extends_used`, `feedback_placeholder`,
  `header_label`, `question_type`); invariant caught + fixed.
- **Idempotency declarations** (R313) — freeze + activate operations
  now have explicit `Idempotent: Yes` docstring + R372 decision-
  three-layer literal lock for `_ATEXIT_GRACE_PERIOD_SECONDS = 1.5`.
- **BUG-fixes from `TODO.md` backlog** (BUG1-BUG6 + bug7) inherited
  from v1.7.10 plus follow-up:
  - BUG4 icon cache + badge (`tests/test_bug4_icon_cache_and_badge.py`)

### Fixed (test infrastructure)

- **`tests/test_changelog_freshness_invariant.py`** — ensures
  CHANGELOG entry exists for every release tag.
- **`tests/test_mcp_tools_doc_consistency.py`** — MCP tool docs
  stay in sync with implementation.
- **`tests/test_notification_health_streak_r145.py`** — `setUp`
  simplified from 13-line manual init to 1-line
  `NotificationManager._create_test_instance()` call.
- **`tests/test_lock_watchdog_full_coverage_r52a.py`** — extended
  to cover new lock acquisitions.
- **`tests/test_silent_failure_audit_r119.py`** — refined silent-
  failure baseline (`tests/data/silent_failure_baseline_r120.json`).
- **`tests/test_runtime_behavior.py`** — runtime contract refinements
  for new endpoints.

### Documentation

- **`docs/methodology-evolution.{md,zh-CN.md}`** — new SSoT for v3.0
  → v3.11 methodology dimensions (R444). 4-layer bilingual structure
  with overview table + per-version detailed descriptions +
  sub-pattern milestones.
- **`docs/troubleshooting.{md,zh-CN.md}`** — bilingual
  troubleshooting guide (R303 / R335 doc-parity locked).
- **`docs/contributor-guide-invariant-tests.{md,zh-CN.md}`** —
  bilingual contributor guide for invariant test patterns (R227 /
  R231 / R400 catalogue + parity).
- **`docs/configuration.{md,zh-CN.md}`** — bilingual configuration
  reference (R394 doc-parity).
- **`docs/code-reviews/cr30.md` … `cr82.md`** — 53 per-cycle code
  reviews documenting cycle-22 through cycle-52 (commits / tests /
  spillover / next-cycle planning).
- **`docs/feature-mining-cycle-{1,2,3,4}.md`** — 4-round structured
  competitive-feature mining backlog from `mcp-feedback-enhanced` /
  `gemini-cli` / `claude-code`.
- **`docs/perf-audit-cycle-{1,2,3}.md`** — 3-round performance audit
  reports.
- **`docs/zhconv-eval.md`** — zhconv library evaluation.
- **README** updated to reflect 7,267 tests + 280+ invariants +
  cycles 1-52 + 3 mermaid sequence diagrams (architecture overview,
  failure & recovery flows, Agent / Glass workflow). Related-
  projects section expanded with feature gap callouts (R357).
- **`README.zh-CN.md`** bilingual parity with main README (R340
  doc-parity).
- **`packages/vscode/README.{md,zh-CN.md}`** bilingual parity (R346).
- **`.github/CONTRIBUTING.{md,zh-CN.md}`** updated for new pre-commit
  norms and invariant-test contribution guide.

### Internal / build

- **`scripts/audit_todo_status.py`** — automated audit of `TODO.md`
  status, ensures completed items get moved to CHANGELOG.
- **`scripts/check_brand_color_consistency.py`** — locks brand
  color tokens (R66 brand color consistency invariant).
- **`scripts/check_i18n_{js,ts}_no_cjk.py`** — JS/TS source must
  not contain hardcoded CJK strings (race-safe after R432 spillover
  fix).
- **`scripts/check_i18n_orphan_keys.py`** — orphan i18n key
  detection across locales.
- **`scripts/ci_gate.py`** — CI gate orchestrator with xdist
  worker count + per-job timeout.

### Records

- **22 consecutive cycles with 0 spillover** (cycle-28 → cycle-49)
  — historical record before being reset by the cycle-50 task.py
  YAML spillover; immediately recovered in cycle-51, then reset
  again in cycle-52 by the bonus R72A pre-existing test isolation
  fix (classified as bonus rather than new invariant error).
- **Methodology v3.6 single-cycle 0 → 4 industrialized** (cycle-35)
  — 4 v3.9 async race contract apps shipped in a single cycle.
- **10+ pattern dimensions running in parallel** (cycle-40 cr70 §3
  through cycle-52): API contract / i18n / perf-baseline /
  decision-three-layer / test-isolation / doc-parity /
  security-header / pydantic-validator / config-default-drift /
  cross-language / async-race / visual-architecture / meta-invariant.
- **i18n single-dimension single-cycle 0 → 4 fully industrialized**
...
- **§3.1 SSE connection-status indicator** — 3-state visual
  (good / degraded / offline) corner badge. Non-intrusive,
  honors `prefers-reduced-motion`. (mining-cycle-1)
- **§3.2 Countdown +60s extend** — UI button + backend
  `POST /api/tasks/<id>/extend` with `max_extends` /
  `min_seconds` / `max_seconds` server-config caps and full
  race-safe `TaskQueue.extend_task_deadline` facade.
  (mining-cycle-1)
- **§3.3 Traditional Chinese (zh-TW) locale** — Web UI +
  VSCode extension. Generated by `scripts/gen_zhtw_from_zhcn.
  py --all` from Simplified Chinese source with project-
  specific `PHRASE_MAP` for Taiwan UI conventions ("反饋" →
  "回饋", "加載" → "載入", etc.). 445-entry `CHAR_MAP_v2`.
  (mining-cycle-1)
- **§3.4 Custom notification sound upload** — drag-drop /
  file-picker, base64 storage in localStorage, MIME type
  whitelist (mp3 / wav / ogg / m4a / flac), 700 KB byte cap,
  30 s duration cap (pre-flight `decodeAudioData` check),
  7 error codes with i18n. (mining-cycle-1)
- **§3.4 (mining-2) One-click copy task_id** — `dblclick` on
  task tab text copies full task_id to clipboard. Dual-path
  (modern `navigator.clipboard` + `document.execCommand`
  legacy fallback). 600 ms visual flash feedback with
  `prefers-reduced-motion` honor. (mining-cycle-2)
- **§3.2 (mining-2) Session-link copy** — `Shift+dblclick` on
  task tab text copies a deep-link URL
  (`?task_id=<id>`). Reuses extracted `_writeToClipboard`
  helper (anti-duplication invariant locked by test).
  (mining-cycle-2)
- **§3.1 (mining-2) Session export UI** — Settings → "Export
  task history" → JSON / Markdown via existing
  `/api/tasks/export` backend (shipped earlier in R125 /
  R125c / R135 but un-discoverable until this UI). Uses
  native `<a download>` for streaming, format whitelist +
  `encodeURIComponent` double defense. (mining-cycle-2)
- **§2.1 (mining-3) Per-task `feedback_placeholder`** —
  borrowed from gemini-cli `ask_user.placeholder` idiom.
  MCP `interactive_feedback` accepts optional
  `feedback_placeholder: str` (200-char clamp server-side);
  frontend `updateFeedbackPlaceholder` helper applies per-
  task placeholder during `switchTask` cache path + async
...
- **`docs/zhconv-eval.md`** — investigation artifact for
  ...
  Decision: not adopting (style control beats character
  coverage). Hybrid design documented as future track
  (trigger: `CHAR_MAP_v2` ≥ 600 + 3 SC-residue bugs).
- **`docs/perf-audit-cycle-1.md`** — backend `TaskQueue` /
  `_SSEBus` / `_upload_helpers` performance audit. Concluded
  optimized to current abstraction limits; 4 deferred
  tracks listed with ROI + trigger conditions.
- **`CONTRIBUTING.md` §2.1 (en + zh-CN)** — codified
  project norm: pre-commit failures must be root-caused,
  never bypassed with `--no-verify`.
- **Code review docs** `cr30.md` → `cr36.md` (7 cycle
  reviews).
- **Task tab `Shift+dblclick` tooltip hint** — modifier
  discoverability for the session-link copy (cycle-2 §3.2).
  i18n key `page.taskTabCopyHint` + English fallback hard-
  coded in JS for resilience.

### Changed

- **BUG1 + BUG2: Web notification de-duplication and i18n** —
  suppress local-save echo when "Configuration file changed"
  fires for our own save; localize the `config_changed` hint
  to the active language.
- **BUG3: settings version no longer shows `vunknown`** —
  use `importlib.metadata` + correct repo-root
  `pyproject.toml` path resolution.
- **BUG4: icon cache + badge cleanup** — offline-resilient
  service worker; removed permanent notification badge from
  task icons; purged dead CSS.
- **BUG5: web auto-recover after backend was unreachable
  at boot** — `Promise.allSettled` boot path + `online`
  event listener.
- **BUG6: config-file path visible in settings** — strip
  stale `data-i18n-value` after writing real path so i18n
  rerun doesn't blank the field.
- **VSCode plugin Resubmit prompt / Feedback suffix length
  parity with Web UI** — both now enforce
  `PROMPT_MAX_LENGTH = 100_000`.
- **Reset settings is now a 2-click confirm flow** —
  destructive action requires explicit confirmation.
- **Removed right-side download button from Web header** —
  feature retired (replaced by Settings → Export task
  history button in mining-2 §3.1).
- **Removed "Send system self-test notification" + activity
  panel** from settings — surfaced too rarely to justify
  the UI weight; data still accessible via API.
- **mining-cycle-2 §0 methodology** — every backlog item
  now requires `rg` + `git log --grep` evidence pasted
  verbatim before classification. Introduced after 3 survey
  misses (§3.5 R131c smart-sort, §3.3 R137 input-height
  memory, §3.1 R125/R125c/R135 session-export).
- **`Task.extend_deadline` and `TaskQueue.extend_task_
  deadline`** now require `max_extends` / `min_seconds` /
  `max_seconds` as **keyword-only mandatory arguments**
  (cr32 §3.3 hardening). Prevents config drift from
  default arguments going stale.
...
- **CR32 §3.1 [medium] Race condition in `extend_deadline`**
  ...
  `threading.Barrier`-synchronized regression test enforces
  the invariant under concurrent stress.
- **CR32 §3.2 [medium] VSCode extension missing
  `locales/zh-TW.json`** — `scripts/gen_zhtw_from_zhcn.py
  --variant vscode` + `--all` flag. Web + VSCode now
  generate together with shared `CHAR_MAP_v2`. Added
  `测`/`缀`/`内`/`达` mappings post-survey.
- **CR33 §8 #1 [medium] Custom sound 30-second memory
  hazard** — pre-flight `decodeAudioData` duration check
  in `saveCustomSoundFromFile`. Adds `duration_too_long`
  error code + i18n.
- **CR35 §8 #2 [low] Color token consistency** — replaced 3
  hardcoded `#c33d3d` literals (custom-sound status / btn-
  danger / copy-flash-err) with `var(--error-500)` design
  token. R66 brand-color audit closure.

### Performance

- **`perf(bfcache-recovery)`** — Web UI now restarts polling
  + verifies SSE health when restored from browser BFCache.
  Eliminates stale countdown after Cursor Glass minimize /
  restore round-trip.

### Removed

- Right-side download button in Web header (replaced by
  Settings export UI).
- "Send system self-test notification" button + activity
  panel in settings.

### Developer / Documentation

- 7 code reviews (`cr30.md` → `cr36.md`).
- 3 mining cycles with hardened methodology
  (`feature-mining-cycle-1.md` / `-2.md` / `-3.md`).
- Investigation artifact for zhconv evaluation
  (`zhconv-eval.md`).
- Backend performance audit (`perf-audit-cycle-1.md`).
- `CONTRIBUTING.md` pre-commit norm codified
  (en + zh-CN).
- `scripts/gen_zhtw_from_zhcn.py` parameterized: `--variant
  web|vscode|all`, deduped CHAR_MAP_v2, executable bit set.
