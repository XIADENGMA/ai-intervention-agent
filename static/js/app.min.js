...
clearAllImages()
if(config){config.has_content=false
...
function startContentPolling(){if(pollingTimeout){console.log('轮询已经在运行，跳过启动')
return}
console.log('开始启动内容轮询...')
scheduleNextPoll()}
function scheduleNextPoll(){pollingTimeout=setTimeout(async()=>{try{const response=await fetch('/api/config')
if(response.status===429){console.warn('遇到速率限制，增加轮询间隔')
...
if(newHasContent&&!currentHasContent){console.log('✅ 检测到新内容，更新页面')
const oldConfig=config
config=newConfig
showContentPage()
updatePageContent(oldConfig)}else if(!newHasContent&&currentHasContent){console.log('📝 内容已清空，显示无内容页面')
config=newConfig
showNoContentPage()
...
if(promptChanged||optionsChanged){console.log('🔄 检测到内容更新，刷新页面')
const oldConfig=config
config=newConfig
updatePageContent(oldConfig)}}else{config=newConfig}
scheduleNextPoll()}catch(error){console.error('轮询错误:',error)
handlePollingError(false)}},currentPollingInterval)
console.log(`内容轮询已安排，间隔${currentPollingInterval}ms`)}
function handlePollingError(isRateLimit){consecutiveErrors++
if(isRateLimit||consecutiveErrors>1){currentPollingInterval=Math.min(basePollingInterval*Math.pow(2,consecutiveErrors),maxPollingInterval)
...
const descriptionElement=document.getElementById('description')
if(descriptionElement){renderMarkdownContent(descriptionElement,config.prompt_html||config.prompt)}
...
optionsContainer.style.display='block'
document.getElementById('separator').style.display='block'}else{optionsContainer.style.display='none'
