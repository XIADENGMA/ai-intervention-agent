...
updateTasksStats(data.stats)}}catch(error){console.error('轮询任务列表失败:',error)}},2000)
...
console.log(`自动设置第一个任务为active: ${activeTaskId}`)}else if(tasks.length===0&&activeTaskId){console.log(`✅ 任务列表已清空，重置 activeTaskId: ${activeTaskId} -> null`)
activeTaskId=null}
renderTaskTabs()
...
startTasksPolling()
setInterval(()=>{if(!tasksPollingTimer){console.warn('⚠️ 任务轮询已停止,自动重新启动')
...
console.log('任务列表已手动刷新')}}catch(error){console.error('手动刷新任务列表失败:',error)}}
