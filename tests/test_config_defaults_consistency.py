...
class TestConfigDefaultsConsistency(unittest.TestCase):
    """防回归：默认配置字典应与模板 config.toml.default 的键保持一致。
    ...
    def test_default_config_keys_match_template(self):
        ...
        template = tomlkit.parse(template_path.read_text(encoding="utf-8")).unwrap()  # type: ignore[union-attr]
        self.assertIsInstance(template, dict)

        with tempfile.TemporaryDirectory() as temp_dir:
            cfg_path = Path(temp_dir) / "config.toml"
