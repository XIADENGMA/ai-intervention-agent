...
背景
----
``static/js/multi_task.js::startTaskCountdown`` 给每个并发任务装一个
...
1. 在 ``startTaskCountdown`` 的 setInterval body 里，把所有 DOM 写入
   框在 ``if (!document.hidden) { ... }`` 内；``calculateRemainingFromDeadline``
   仍正常执行（deadline 是绝对时间，autoSubmit 的判定不能因为页面
   hidden 而推迟）。
...
1.  **源码不变量**：``startTaskCountdown`` body 包含
    ``document.hidden`` 检查，且 DOM 写入分支被框在
    ``if (!documentHidden) { ... }`` 内；``calculateRemainingFromDeadline``
    必须在该 if 之外（保持每秒更新）。
...
# ---------------------------------------------------------------------------
# 1. startTaskCountdown 源码不变量
# ---------------------------------------------------------------------------


class TestStartTaskCountdownHiddenSkipsDOM(unittest.TestCase):
    """``startTaskCountdown`` 的 setInterval body 必须在 hidden 时跳过 DOM 操作。"""

    @classmethod
    def setUpClass(cls) -> None:
        cls.source = _read_source()
        cls.body = _extract_named_function_body(cls.source, "startTaskCountdown")
    ...
    def test_body_checks_document_hidden(self) -> None:
        ...
        self.assertRegex(
            self.body,
            r"document\.hidden",
            "startTaskCountdown 必须检查 document.hidden 才能跳过隐藏时的 DOM 写入",
        )
    ...
    def test_calculate_remaining_runs_outside_hidden_guard(self) -> None:
        """``calculateRemainingFromDeadline`` 必须在 ``if (!documentHidden)`` 之**前**调用。

        why：deadline 是绝对时间，autoSubmit 的判定不能因为页面 hidden
        而推迟一秒（会让到期任务被推迟 0-N 秒提交）。
        """
        calc_idx = self.body.find("calculateRemainingFromDeadline()")
        guard_idx = self.body.find("if (!documentHidden)")
        self.assertGreater(calc_idx, -1, "找不到 calculateRemainingFromDeadline 调用")
        self.assertGreater(guard_idx, -1, "找不到 if (!documentHidden) 守护")
        self.assertLess(
            calc_idx,
            guard_idx,
            "calculateRemainingFromDeadline 必须在 hidden 守护之前调用（deadline 是绝对时间，"
