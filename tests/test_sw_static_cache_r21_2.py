...
2. **新增静态资源缓存**——拦截白名单路径下的 GET 请求，命中 cache 立刻
   返回并后台 revalidate，未命中走网络后异步写入 cache，重复访问 RTT ≈ 0。
...
class TestServiceWorkerStructure:
    ...
    def test_static_hit_uses_wait_until_background_revalidate(self) -> None:
        """R459：静态 cache hit 必须即时返回，同时用 ``event.waitUntil`` 保活
        后台刷新，避免 cache-first 永久卡住未版本化资源的旧副本。"""
        text = _read(SW_PATH)
        assert "event.respondWith(handleCacheFirst(request, event))" in text
        assert "refreshStaticCache(request, cache)" in text
        assert re.search(r"event\.waitUntil\(\s*refreshPromise\s*\)", text), (
            "R459 期望 cache hit 走 stale-while-revalidate：返回 cached response "
            "的同时用 event.waitUntil(refreshPromise) 保活后台网络刷新。"
        )
